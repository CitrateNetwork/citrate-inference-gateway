...
struct MockChainQueries { provider_endpoint: String }
#[async_trait]
impl ChainQueries for MockChainQueries {
    async fn resolve_model_name(&self, _: &str) -> Result<H256, citrate_gateway::GatewayError> { Ok(H256::from([0xab; 32])) }
    async fn estimate_cost(&self, _: H256, _: u32, _: u32, _: u8) -> Result<U256, citrate_gateway::GatewayError> { Ok(U256::from(1_000_000_000_000_000_000u128)) }
    async fn list_providers(&self, _: H256) -> Result<Vec<ProviderInfo>, citrate_gateway::GatewayError> {
        Ok(vec![ProviderInfo {
...
struct HonestMockChain { facilitator: H160, settled: Mutex<HashSet<H256>> }
impl HonestMockChain { fn new(f: H160) -> Self { Self { facilitator: f, settled: Mutex::new(HashSet::new()) } } }
#[async_trait]
...
async fn spawn_gateway(provider: SocketAddr) -> (SocketAddr, Arc<ApiKeyStore>) {
    ...
    let app = build_router_with_auth(config, queries, chain, operator_secret(), facilitator, keys.clone()).await;
...
async fn metrics_endpoint_returns_text_exposition() {
    ...
    let resp = reqwest::get(&url).await.expect("metrics");
...
async fn chat_request_increments_counter() {
    ...
    // Scrape /metrics and confirm the success counter moved.
    let body = reqwest::get(&format!("http://{}/metrics", gw))
        .await
        .expect("metrics")
        .text()
        .await
    ...
    let success_line = body
        .lines()
        .find(|l| l.starts_with("gateway_chat_requests_total{") && l.contains("outcome=\"success\""))
    ...
    let val: f64 = success_line
        ...
        .parse()
        .expect("f64");
    assert!(val >= 1.0, "counter should be ≥ 1, got {} in line {}", val, success_line);
}
